...
def move(dir_code: str, ctx: Dict[str, Any]) -> None:
    ...
    # Record start timestamp to measure end-to-end render latency for movement.
    try:
        ctx["_move_lag_probe"] = {
            "start": time.perf_counter(),
            "dir": dir_code,
            "from": (int(year), int(x), int(y)),
        }
    except Exception:
    ...
    # Defer persistence to the end-of-command checkpoint; keep runtime state in sync.
    pstate.normalize_player_state_inplace(canonical_state)
    ctx["player_state"] = canonical_state
    runtime_player = pstate.ensure_player_state(ctx)
    if isinstance(runtime_player, MutableMapping):
        runtime_player["_dirty"] = True
    # When no turn scheduler is present (e.g., unit tests calling move directly),
    # persist immediately so subsequent commands see the updated position.
    scheduler = ctx.get("turn_scheduler") if isinstance(ctx, Mapping) else None
    if scheduler is None:
        try:
            pstate.save_state(canonical_state, reason="move-immediate")
            runtime_player = pstate.ensure_player_state(ctx)
            if isinstance(runtime_player, MutableMapping):
                runtime_player["_dirty"] = False
        except Exception:
            LOG.debug("Failed immediate player save after move", exc_info=True)

    pstate.sync_runtime_position(ctx, new_pos)
    try:
        ions_after = pstate.get_ions_for_active(canonical_state) if isinstance(canonical_state, Mapping) else ions_before
    except Exception:
