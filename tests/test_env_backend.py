...
def test_backend_defaults_to_sqlite_and_logs(monkeypatch, caplog) -> None:
...
def test_backend_honours_sqlite(monkeypatch, tmp_path, caplog) -> None:
...
def test_backend_ignores_non_sqlite_values(monkeypatch, caplog) -> None:
    ...
    try:
        ...
        message = caplog.records[0].message
        assert "backend=sqlite" in message
        assert str(env_module.get_state_database_path()) in message
    finally:
        monkeypatch.delenv("MUTANTS_STATE_BACKEND", raising=False)
        monkeypatch.delenv("GAME_STATE_ROOT", raising=False)
        _reload_state()
